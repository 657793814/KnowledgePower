...
export async function seedSymmetricExpressionsModel() {
  ...
  await sn('MATH-02-019', '整体代入与对称式模型', '已知基本对称式求高次对称式的值', 'math', '式', '初中', 4, 219, 'model',
    ...
    modelContent({
      ...
      recognition: [
        '题目给出 x+y 和 xy 的值，要求 x²+y²、x³+y³ 等高次式',
        '出现 x+1/x 的形式，考虑平方/立方构造',
        '轮换对称式，尝试用基本对称式表示',
        '已知两数和与积，可逆用韦达定理构造二次方程',
      ],
      principle: '基本对称式 s=x+y, p=xy。通过乘法公式和代数变形，可将高次对称式用 s,p 表示。关键公式：x²+y²=s²-2p，x³+y³=s(s²-3p)，x⁴+y⁴=s⁴-4s²p+2p²。对于形如 x+1/x 的式子，平方可得 x²+1/x²，再平方可得 x⁴+1/x⁴。',
      principleFormulas: [
